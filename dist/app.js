...
function renderCourse(){
 ...
 if(relayRestored()&&calibrated){$('#routePanel h2').textContent='Recursion beacon found';$('#exploreIntro p').textContent='Your scanner has resolved the recursion field. Follow the violet beam.';$('#investigateRelay').textContent='Follow recursion beacon'}
 if(!active)return;
...
function renderRecursion(){
 ...
 if(courseCalibrated()&&solved){$('#routePanel h2').textContent='Golden anomaly mapped';$('#exploreIntro p').textContent='The branch count unlocked an optional golden-angle archive.';$('#investigateRelay').textContent='Investigate golden surprise'}
 if(!active)return;
...
function renderGolden(){
 ...
 if(recursionSolved()&&solved){$('#routePanel h2').textContent='Ratio beacon awake';$('#exploreIntro p').textContent='The golden-angle archive now points toward the Fibonacci ratio beacon.';$('#investigateRelay').textContent='Follow ratio beacon'}
 if(!active)return;
 $('#goldenQuestion').textContent=`The archive is checking the first 12 turns for a small repeating spoke pattern. Tune the spiral near ${GOLDEN_ANGLE.toFixed(3)} degrees and use enough seeds to see the gap pattern.`;
 $('#goldenTransmit').textContent=`Test ${values.angle.toFixed(1)}° spiral`;
 $('#goldenTransmit').disabled=!recursionSolved()||solved;
...
$('#goldenTransmit').onclick=()=>{
 if(current?.id!=='golden'||!recursionSolved()||goldenSolved())return;
...
function renderRelay(){
 ...
 $('#routePanel h2').textContent=restored?'Signal restored':'Restore the signal';
 $('#exploreIntro p').textContent=restored?'The beacon is clear. Follow it to the Circular Signal.':'The relay has lost its signal. Can you silence the interference?';
 $('#investigateRelay').textContent=restored?'Follow beacon':'Investigate relay';
 if(!active)return;
...
function nextLead(){return !relayRestored()?'waves':!courseCalibrated()?'unitcircle':!recursionSolved()?'fractals':!goldenSolved()?'golden':discoveries.find(d=>!visited.includes(d.id))?.id||'fibonacci'}
